...
babrat::wal::WALManager::WALManager(const std::string &path, const WALOptions& options):
        options_(options),
...
void babrat::wal::WALManager::writePack(WriteBatch *batch) {
    ...
    for (const auto & entry : batch->entries) {
        ...
        auto [out, epos] = format_handler_->appendEntry(
        seg->ebuf,
             entry.index,
             {datas.begin(), datas.begin() + static_cast<std::ptrdiff_t>(entry.size)}
         );
        ...
        if (seg->ebuf.size() >= options_.getSegmentSize()) {
            ...
            file_.write(reinterpret_cast<const char*>(seg->ebuf.data() + mark),
                        static_cast<std::streamsize>(seg->ebuf.size() - mark));
        ...
        datas.erase(datas.begin(), datas.begin() + static_cast<std::ptrdiff_t>(entry.size));
...
size_t babrat::wal::WALManager::findSegment(std::uint64_t index) const {
    ...
    return (it == segments_.begin())
        ? 0
        : static_cast<size_t>(std::distance(segments_.begin(), it) - 1);
...
void babrat::wal::WALManager::load(){
    ...
    if (start_idx != -1) {
        ...
        for (int i = 0; i < start_idx; i++) {
            if (!fs::remove(segments_[static_cast<size_t>(i)]->path)) {
    ...
    if (end_idx != -1) {
        ...
        for (size_t i = segments_.size() - 1; i > static_cast<size_t>(end_idx); i--) {
...
void babrat::wal::WALManager::loadSegmentEntries(const std::shared_ptr<Segment>& seg) {
    ...
    seg->ebuf.resize(static_cast<size_t>(file.tellg()));

    file.seekg(0);
    if (!file.read(reinterpret_cast<char*>(seg->ebuf.data()),
          static_cast<std::streamsize>(seg->ebuf.size()))) {
    ...
    while (pos < seg->ebuf.size()) {
        ...
        std::vector<uint8_t> entry_data(
         std::next(seg->ebuf.begin(), static_cast<std::ptrdiff_t>(pos)),
         seg->ebuf.end()
        );

        size_t n = static_cast<size_t>(format_handler_->loadNextEntry(entry_data));

        if (pos + n > seg->ebuf.size()) {
...
void babrat::wal::WALManager::truncateFrontIndex(uint64_t index){
    ...
    std::vector<BPos> epos_slice(
      s->epos.begin() + static_cast<std::ptrdiff_t>(start_idx),
      s->epos.end()
    );

    std::vector<uint8_t> ebuf_slice(
     s->ebuf.begin() + static_cast<std::ptrdiff_t>(epos_slice[0].pos),
     s->ebuf.end()
    );
    ...
    fs::path temp_path = path_ / "TEMP";
    {
        ...
        temp_file.write(
            reinterpret_cast<const char*>(ebuf_slice.data()),
            static_cast<std::streamsize>(ebuf_slice.size())
        );
    ...
    try {
        ...
        segments_.erase(segments_.begin(),
               segments_.begin() + static_cast<std::ptrdiff_t>(seg_idx));
        ...
        if (seg_idx == segments_.size() - 1) {
            ...
            if (file_.tellp() != static_cast<std::streamoff>(s->ebuf.size())) {
...
void babrat::wal::WALManager::truncateBackIndex(uint64_t index) {
    ...
    std::vector<BPos> epos_slice(s->epos.begin(),
        s->epos.begin() +
        static_cast<std::vector<BPos>::difference_type>(end_idx));

    std::vector<uint8_t> ebuf_slice(
     s->ebuf.begin(),
     s->ebuf.begin() + static_cast<std::vector<uint8_t>::difference_type>(epos_slice.back().end)
    );
...
void babrat::wal::WALManager::checkState() const {
    if (corrupt_) throw CorruptWalException();
    if (closed_) throw ClosedFileWalException();
}
