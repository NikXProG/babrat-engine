...
namespace babrat {
    ...
    namespace container {
        ...
        /**
          * @brief adapter over `std::map<Key, Value, Compare>` container implementing a Red-Black tree.
          ...
          *
          * @details This class implements a balanced search tree (Red-Black tree) that maintains
          * its elements in sorted order according to the Compare function.
          *
          * @see For more common about Red-Black trees, see:
          *      <a href="https://en.wikipedia.org/wiki/Red–black_tree">Red-Black Tree on Wikipedia</a>
          */
        ...
        class RedBlackTree final : public SearchTree<Key,  Value> {
            ...
            std::map<Key, Value, Compare> * m_tree;

        public:

            RedBlackTree() : m_tree(new std::map<Key, Value, Compare>()){}
        ...
        public:
            ...
            /**
              * @brief Iterator class for RedBlackTree implementing mutable iteration.
              *
              * @details Provides forward iteration capability over the RedBlackTree 's key-value pairs.
              */
            ...
            public:
                ...
                bool operator!=(const typename SearchTree<Key,Value>::IteratorBase& other) const override {
            ...
            /**
              * @brief Const iterator class for RedBlackTree implementing read-only iteration.
              *
              * @details Provides forward iteration capability over the RedBlackTree's key-value pairs
            ...
            typename SearchTree<Key, Value>::Iterator begin() override {
                return typename SearchTree<Key, Value>::Iterator(
                        std::make_unique<RedBlackTreeIterator>(m_tree->begin())
                );
            }
            ...
            typename SearchTree<Key, Value>::Iterator end() override {
                return typename SearchTree<Key, Value>::Iterator(
                        std::make_unique<RedBlackTreeIterator>(m_tree->end())
                );
            }
            ...
            typename SearchTree<Key, Value>::ConstIterator cbegin() const override {
                return typename SearchTree<Key, Value>::ConstIterator(
                        std::make_unique<RedBlackTreeConstIterator>(m_tree->begin())
                );
            }
            ...
            typename SearchTree<Key, Value>::ConstIterator cend() const override {
                return typename SearchTree<Key, Value>::ConstIterator(
                        std::make_unique<RedBlackTreeConstIterator>(m_tree->end())
                );
            }
        ...
        private:
            ...
            void copyFrom(
                ...
                m_tree =
                        new std::map<Key, Value, Compare>(
                                *dynamic_cast<std::map<Key, Value, Compare> *>(other.m_tree));
            ...
            void moveFrom(
                ...
                m_tree =
                        new std::map<Key, Value, Compare>(
                                std::move(*dynamic_cast<std::map<Key, Value, Compare> *>(other.m_tree)));
            }
